...
- 连续天数计算隔离在 `resolveStreakDays()`，失败时只回退连续天数，不得重置可见复盘数量。
...
1. 结构约束：禁止恢复 `StatItem(...)`、嵌套 `dashboardStats`、旧配置状态卡和诊断面板。
...
- 首页统计新增字段时，先补 `ReviewProjectService` 的纯数据口径，再补 `verify_home_stats.mjs` 的行为用例，最后改 UI。
