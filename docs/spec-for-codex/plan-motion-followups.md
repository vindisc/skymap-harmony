...
| V5-P1-1 删除后残余闪烁追加修复 | 代码完成，待真机 | 根因是成功删除已从数据源移除后立即执行 `endCollapse` 与 `markCardVisible`，ArkUI 可能在 `ForEach` 提交删除前短暂恢复被删卡片。成功路径现只记录待清理 key，延迟到 280ms 稳定窗口末尾再回收视觉状态；异常路径仍立即恢复原卡片。待复盘空状态、历史空状态和筛选空状态在删除稳定窗口结束后再淡入，避免删除最后一项时硬切闪白。门禁新增“成功路径禁止恢复被删卡片”与三类空状态过渡断言。 |
| V5-P1-1 录屏逐帧 RCA | 代码完成，待同场景复测 | 对 5.97 秒、约 55fps 录屏按 30fps 拆帧后确认：约 4.40–4.70 秒标题、筛选和底栏稳定，仅全部幸存卡片同步淡出再淡入；实际根因是动态 `ListItem` 内的 `StaggeredEnter.aboutToAppear()` 在删除重建时把 `entered` 重置为 false。待复盘与历史两条动态列表已直接移除 `StaggeredEnter` 包装，保留卡片自身 collapse/ripple 动效；门禁明确禁止 `ProjectDetailPage` 恢复可重播入场层。 |
| V5 追加：全部聚合与搜索字号 | 代码完成，待真机 | “全部”由仅显示历史记录改为先显示匹配的待复盘卡片、再显示全部历史状态；成立/待判断/不成立仍只筛历史，待复盘仍保留独立视图。混合列表继续按历史数量分页，搜索同时匹配两类数据，只有两类均无结果才显示空状态；待复盘与历史删除改用同一互斥锁，避免混合列表同屏操作争用星河和收缩状态。搜索占位文字使用 `AppTypography.meta`（12fp），实际输入仍保持 14fp。 |
