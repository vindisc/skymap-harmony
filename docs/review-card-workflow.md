...
如果已经在「复盘设置」中配置家庭存储，阅读页也可以直接点击「上传家庭存储」。WebDAV 模式会把同一份 `*.review.json` 上传到配置的远程目录；SMB 模式会打开系统保存面板，用户选择系统文件中的 SMB 共享目录。
...
- SMB：App 使用系统保存面板，由系统文件服务访问 SMB 共享目录。
...
SMB 模式不在 App 内实现 SMB 协议栈，也不保存 SMB 账号密码。账号、共享目录和访问授权由系统文件服务处理。

1. 先在系统文件服务中确认可以访问华为家庭存储的 SMB 共享。
2. 进入「复盘设置」。
3. 选择 `SMB` 并保存。
4. 回到阅读页点击「上传家庭存储」。
5. 在系统保存面板中选择 SMB 共享目录。

SMB 保存仍然使用 `*.review.json` 文件名规则，推荐保存到：

```text
摄影资料库/Reviews
```

系统保存面板仍然保留；家庭存储上传失败不会影响本地复盘记录或系统保存能力。
