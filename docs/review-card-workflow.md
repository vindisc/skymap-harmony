...
如果已经在「复盘设置」中配置家庭存储 WebDAV，阅读页也可以直接点击「上传家庭存储」，App 会把同一份 `*.review.json` 上传到配置的远程目录。
...
当前家庭存储连接使用 WebDAV MVP：

1. 进入「复盘设置」。
2. 填写 WebDAV 地址、用户名、密码或 Token、远程目录。
3. 点击「测试连接」确认目录可访问。
4. 回到阅读页点击「上传家庭存储」。

上传使用 `PUT` 写入配置目录下的 `*.review.json`。如果目录不存在，当前版本不会自动创建目录，用户需要先在家庭存储中创建，例如：

```text
摄影资料库/Reviews
```

系统保存面板仍然保留；WebDAV 上传失败不会影响本地复盘记录或系统保存能力。
