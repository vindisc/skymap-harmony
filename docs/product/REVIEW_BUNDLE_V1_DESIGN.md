...
- “导出复盘包”是当前推荐的家庭存储和 Mac 接力方式。
- “导出图片”只生成当前复盘卡片图片，不包含复盘数据文件。
- “导出 review.json”只导出复盘数据文件，不包含图片，属于高级兼容入口。
- 不再在普通路径单独显示“上传家庭存储”，避免和“导出复盘包”重复。
- “复制复盘数据”只复制当前复盘的文本数据，不能泛称为“复制数据”。
