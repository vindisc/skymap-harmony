...
- 导出复盘长图
- 导出 `review.json`
- SMB 上传 `review.json` 到家庭存储

补充说明：

- 上述 SMB 恢复能力已经回到当前代码基线。
- 但是否进入 Beta 对外承诺，仍取决于当前轮次真机回归结果。
...
- 不把 SMB 自动同步、WebDAV、Review Bundle 或 manifest 当成当前 Beta 主链路。
- 当前仅恢复 SMB 最小上传能力，不继续在本轮把家庭存储扩成完整同步系统。
- SMB 代码已经恢复到当前基线，但仍要等真机回归结果才能决定是否进入对外 Beta 承诺。
