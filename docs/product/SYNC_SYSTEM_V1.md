...
- Harmony 当前代码基线已恢复最小 SMB 家庭存储能力：家庭存储页配置、连接测试、阅读页上传 `review.json`。
- WebDAV 仍只存在于历史分支，不在本轮恢复范围内。
- 因此，当前 Product Layer 应把 SMB 视为“代码已恢复、但仍需真机验收的最小家庭存储能力”，而不是“当前已稳定支持的完整同步系统”。
...
当前 Harmony 基线已具备的能力只有：

- 设置 SMB 家庭存储
- 测试 SMB 连接
- 上传当前复盘 `review.json` 到 SMB
...
也就是说，当前恢复的是“家庭存储最小上传能力”，不是 Sync System v1 的完整交付。
