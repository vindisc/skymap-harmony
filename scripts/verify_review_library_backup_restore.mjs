...
const backupCenterSource = fs.readFileSync('entry/src/main/ets/pages/BackupCenterPage.ets', 'utf8');
...
assert(backupSource.includes('sha256(JSON.stringify(envelope.payload)) !== envelope.checksum'), '恢复前必须校验 checksum。');
...
assert(myPageSource.includes("title: '备份与恢复'"), '我的页必须提供备份中心入口。');
assert(myPageSource.includes('BACKUP_CENTER_PAGE'), '我的页必须保持备份中心路由。');
assert(!myPageSource.includes("title: '备份全部复盘'"), '我的页不得重复承载完整备份操作。');
assert(!myPageSource.includes("title: '从备份恢复'"), '我的页不得重复承载恢复操作。');
assert(backupCenterSource.includes("title: '备份全部复盘'"), '备份中心必须提供完整备份入口。');
assert(backupCenterSource.includes("title: '从备份恢复'"), '备份中心必须提供恢复入口。');
assert(backupCenterSource.includes('ReviewLibraryBackupService.exportToUserFile'), '备份中心必须保留导出逻辑。');
assert(backupCenterSource.includes('ReviewLibraryBackupService.restoreInspectedBackup'), '备份中心必须保留恢复逻辑。');
