...
assert(files.feedback.includes("REVIEW_FLOW_IMPORT_FAILED_TEXT: string = '照片导入失败，请重试'"), 'Import failure copy constant must match UX baseline.');
...
assert(!files.my.includes('HomeStorageService.loadSettings'), 'My page must not expose non-Beta home storage status.');
assert(!files.my.includes("title: '同步中心'"), 'My page must hide non-Beta sync center entry.');
assert(!files.my.includes("title: '家庭存储'"), 'My page must hide non-Beta home storage entry.');
